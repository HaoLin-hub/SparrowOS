...
static void wash_path(char *old_abs_path, char *new_abs_path) {
    ...
    // name数组存储路径中解析出来的“各层目录名”
    char name[MAX_FILE_NAME_LEN] = {0};
    // 解析old_abs_path, 将上层目录名存入name数组, 剩余的子路径返回到sub_path中
    char *sub_path = old_abs_path;
    sub_path = path_parse(sub_path, name);
    if (name[0] == 0) { // 若name数组依然为0, 说明old_abs_path只含有"/", 直接将"/"存入new_abs_path后返回
        new_abs_path[0] = '/';
        new_abs_path[1] = 0;
        return;
    }

    new_abs_path[0] = 0;         // 避免传给new_abs_path的缓冲区不干净, 并在new_abs_path首字符插入一个'/'作为绝对路径起始符
    strcat(new_abs_path, "/");

    while (name[0]) {
        // 如果是上一级目录“..”
        if (!strcmp("..", name)) {
            char* slash_ptr = strrchr(new_abs_path, '/');  // 找到new_abs_path的最后一个路径分隔符‘/’
            // 如果slash_ptr指向的不是new_abs_path首字符地址,就将最右边的'/'替换为0, 这样便去除了new_abs_path中最后一层路径,相当于到了上一级目录
            if (slash_ptr != new_abs_path) {    // 如new_abs_path为“/a/b”,".."之后则变为“/a”
                *slash_ptr = 0;
            } else {  // 若new_abs_path中只有1个'/',即表示已经到了顶层目录,就将下一个字符置为结束符0
                *(slash_ptr + 1) = 0;
            }
        } else if (strcmp(".", name)) {   // 如果路径不是‘.’,就将name拼接到new_abs_path
            if (strcmp(new_abs_path, "/")) {      // 如果new_abs_path不等于"/", 说明new_abs_path目前是“/a”这种格式, 就拼接一个"/",此处的判断是为了避免路径开头变成这样"//"
                strcat(new_abs_path, "/");
            }
...
/* path是用户在命令行键入的路径, 将path处理成不含..和.的绝对路径,存储在final_path */
