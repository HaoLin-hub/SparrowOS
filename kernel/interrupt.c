...
static void pic_init(void) {
   ...
   /* IRQ2用于级联从片，必须打开，否则无法响应从片上的中断，其他则关闭 */
   outb (PIC_M_DATA, 0xf8);
   outb (PIC_S_DATA, 0xbf); // 打开IRQ14响应硬盘控制器的中断
/* 此时打开的中断有键盘中断，和时钟中断以及级联从片的IRQ2 */
...
/* 通用的中断处理函数,一般用在异常出现时的处理, 只接受一个参数：中断向量号 */
static void general_intr_handler(uint8_t vec_nr) {
   ...
   // 将光标置为0, 从屏幕左上角清出一片打印异常信息的区域, 进而方便阅读
   set_cursor(0);
   int cursor_pos = 0;
    ...
    if(vec_nr == 14) { // 若为Page fault, 将缺失的地址打印出来并悬停
        int page_fault_vaddr = 0;
        asm volatile ("movl %%cr2, %0" : "=r"(page_fault_vaddr));    // cr2是存放造成page fault的虚拟地址
        put_str("\npage fault address is ");
        put_int(page_fault_vaddr);
    }
    put_str("\n!!!!!!      exception message end      !!!!!\n");
   // 能进入中断处理程序就表示已经处在关中断的情况, 不会再出现调度进程的情况，故下面的死循环不会再被中断
    while (1);
}
...
enum intr_status intr_get_status(){
    uint32_t eflags = 0;
    // 利用宏获取当前Eflags寄存器的状态存入C语言变量eflags中
    GET_EFLAGS(eflags);
    // 判断当前Eflags寄存器状态的IF位是否为1
